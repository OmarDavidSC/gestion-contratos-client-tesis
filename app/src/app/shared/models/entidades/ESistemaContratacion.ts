...
export class ESistemaContratacion {
  Id: any;
  Nombre: string;
  Habilitado: Boolean;
  ...
  public static parseJson(element: any): ESistemaContratacion {    
    const objeto = new ESistemaContratacion();
    objeto.Id = SPParse.getString(element["id"]);
    objeto.Nombre = SPParse.getString(element["nombre"]);
  ...
  public static parseJsonList(elements: any): ESistemaContratacion[] {

    const lista: ESistemaContratacion[] = [];

    for (const element of elements) {
      const objeto = ESistemaContratacion.parseJson(element);
      lista.push(objeto);
    }
