...
export class EMoneda {
  ...
  public static parseJson(element: any): EMoneda {    
    const objeto = new EMoneda();
    objeto.Id = SPParse.getString(element["id"]);
    objeto.Nombre = SPParse.getString(element["nombre"]);
  ...
  public static parseJsonList(elements: any): EMoneda[] {

    const lista: EMoneda[] = [];

    for (const element of elements) {
      const objeto = EMoneda.parseJson(element);
      lista.push(objeto);
    }
