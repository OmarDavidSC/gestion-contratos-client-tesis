...
export class ETipoAdenda {
  ...
  public static parseJson(element: any): ETipoAdenda {    
    const objeto = new ETipoAdenda();
    objeto.Id = SPParse.getString(element["id"]);
    objeto.Nombre = SPParse.getString(element["nombre"]);
  ...
  public static parseJsonList(elements: any): ETipoAdenda[] {

    const lista: ETipoAdenda[] = [];

    for (const element of elements) {
      const objeto = ETipoAdenda.parseJson(element);
      lista.push(objeto);
    }
