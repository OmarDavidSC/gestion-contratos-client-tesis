...
export class EArea {
  Id: any;
  Nombre: string;
  UsuarioResponsable: EUsuarioLookup;
  Habilitado: Boolean;
  ...
  constructor() {
    this.Id = 0;
    this.Nombre = "";
    this.UsuarioResponsable = new EUsuarioLookup();
  ...
  public static parseJson(element: any): EArea {    
    const objeto = new EArea();
    objeto.Id = SPParse.getString(element["id"]);
    objeto.Nombre = SPParse.getString(element["nombre"]);
    objeto.UsuarioResponsable = EUsuarioLookup.parseJson(element["usuarioResponsable"]);
  ...
  public static parseJsonList(elements: any): EArea[] {

    const lista: EArea[] = [];

    for (const element of elements) {
      const objeto = EArea.parseJson(element);
      lista.push(objeto);
    }
