...
export class ETipoPoliza {
  ...
  public static parseJson(element: any): ETipoPoliza {    
    const objeto = new ETipoPoliza();
    objeto.Id = SPParse.getString(element["id"]);
    objeto.Nombre = SPParse.getString(element["nombre"]);
  ...
  public static parseJsonList(elements: any): ETipoPoliza[] {

    const lista: ETipoPoliza[] = [];

    for (const element of elements) {
      const objeto = ETipoPoliza.parseJson(element);
      lista.push(objeto);
    }
