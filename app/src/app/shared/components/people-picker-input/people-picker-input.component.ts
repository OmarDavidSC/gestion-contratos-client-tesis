...
import { Component, Input, Output, ElementRef, EventEmitter, ViewChild, OnDestroy, Optional, Self } from '@angular/core';
...
import { Variable } from '../../../../../node_modules/@angular/compiler/src/render3/r3_ast';
import { Constantes } from 'src/app/shared/utils/Constantes';

/** Custom `MatFormFieldControl` for telephone number input. */
@Component({
  selector: 'people-picker-input',
  templateUrl: './people-picker-input.component.html',
  styleUrls: ['./people-picker-input.component.scss'],
  providers: [{ provide: MatFormFieldControl, useExisting: PeoplePickerInputComponent }],
  host: {
...
export class PeoplePickerInputComponent implements ControlValueAccessor, MatFormFieldControl<User[]>, OnDestroy {

  @Output() onRemove: EventEmitter<boolean> = new EventEmitter<boolean>();

  static nextId = 0;

  selectValue: User = new User();
  ...
  constructor(
    ...
    /* Control - Inicio */
    this.peoples = [];
    this.peopleCtrl.valueChanges.pipe(debounceTime(400))
  ...
  remove(people: any): void {
    ...
    if (index >= 0) {
      ...
      this.peoples = [...this.peoples];//this.peoples.splice(index, 1);
  ...
  selected(event: MatAutocompleteSelectedEvent): void {
    ...
    var that = this;
    this.userService.getUserIdEnsure(event.option.value.Key).then(function (user_id) {
  ...
  @Input()
  get nombregrupo(): string { return this._nombregrupo; }
  ...
  @Input()
  get usuariosseleccionados(): User[] { return this._usuariosseleccionados; }
  set usuariosseleccionados(value: User[]) {
    this._usuariosseleccionados = value;
    const prefixUrl = environment.webAbsoluteUrl + "/_layouts/15/userphoto.aspx?size=S&accountname=";
  ...
  @Input()
  get limpiar(): boolean { return this._limpiar; }
  set limpiar(value: boolean) {
    this._limpiar = value;
  }
