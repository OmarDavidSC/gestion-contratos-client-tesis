...
@Component({
  selector: 'app-modal-fomulario-moneda',
  templateUrl: './modal-fomulario-moneda.component.html',
  styleUrls: ['./modal-fomulario-moneda.component.scss']
})
export class ModalFormularioMonedaComponent implements OnInit {
  ...
  constructor(
    @Inject(MAT_DIALOG_DATA) public data: EMoneda,
    private formBuilder: FormBuilder,
    private monedaService: MonedaService,
    private spinnerService: NgxSpinnerService,
    private usuarioService: UsuarioService,
    public dialogRef: MatDialogRef<ModalFormularioMonedaComponent>
  ) { }

  ngOnInit(): void {
    this.TituloPopup = this.data ? 'Editar Moneda' : 'Registrar Moneda';
    ...
    if (this.data) {
      sweet2.question({
        ...
        onOk: async () => {
          ...
          setTimeout(async () => {
            await this.monedaService.updateItem(datos);
      ...
      sweet2.question({
        ...
        onOk: async () => {
          sweet2.loading();
          setTimeout(async () => {
            const response = await this.monedaService.addItem(datos);
            sweet2.loading(false);
            if (response.message) {
              Swal.fire({
