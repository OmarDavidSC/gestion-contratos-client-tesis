...
@Component({
  selector: 'app-modal-formulario-tipo-contrato',
  templateUrl: './modal-formulario-tipo-contrato.component.html',
  styleUrls: ['./modal-formulario-tipo-contrato.component.scss']
})
export class ModalFormularioTipoContratoComponent implements OnInit {
  ...
  constructor(
    @Inject(MAT_DIALOG_DATA) public data: ETipoContrato,
    private formBuilder: FormBuilder,
    private tipoContratoService: TipoContratoService,
    private spinnerService: NgxSpinnerService,
    private usuarioService: UsuarioService,
    public dialogRef: MatDialogRef<ModalFormularioTipoContratoComponent>
  ) { }

  ngOnInit(): void {
    this.TituloPopup = this.data ? 'Editar Tipo de Contrato' : 'Registrar Tipo de Contrato';
    ...
    if (this.data) {
      sweet2.question({
        ...
        onOk: async () => {
          ...
          setTimeout(async () => {
            await this.tipoContratoService.updateItem(datos);
      ...
      sweet2.question({
        ...
        onOk: async () => {
          sweet2.loading();
          setTimeout(async () => {
            const response = await this.tipoContratoService.addItem(datos);
            sweet2.loading(false);
            if (response.message) {
              Swal.fire({
