...
@Component({
  selector: 'app-modal-formulario-tipo-documento',
  templateUrl: './modal-formulario-tipo-documento.component.html',
  styleUrls: ['./modal-formulario-tipo-documento.component.scss']
})
export class ModalFormularioTipoDocumentoComponent implements OnInit {
  ...
  constructor(
    @Inject(MAT_DIALOG_DATA) public data: ETipoDocumento,
    private formBuilder: FormBuilder,
    private tipoDocumentoService: TipoDocumentoService,
    private spinnerService: NgxSpinnerService,
    private usuarioService: UsuarioService,
    public dialogRef: MatDialogRef<ModalFormularioTipoDocumentoComponent>
  ) { }
    ...
    if (this.data) {
      sweet2.question({
        ...
        onOk: async () => {
          ...
          setTimeout(async () => {
            await this.tipoDocumentoService.updateItem(datos);
      ...
      sweet2.question({
        ...
        onOk: async () => {
          sweet2.loading();
          setTimeout(async () => {
            const response = await this.tipoDocumentoService.addItem(datos);
            sweet2.loading(false);
            if (response.message) {
              Swal.fire({
